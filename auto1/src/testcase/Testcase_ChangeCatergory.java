...
public class Testcase_ChangeCatergory {
...
@SuppressWarnings("deprecation")
	...
	public void test() throws Exception{
		...
		objGeneralDiscussion.Changing();
		Reporter.log("discussion created successful");
		
		driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
		String actualText = objGeneralDiscussion.getDiscusionDashboard().trim().toLowerCase();
		if(actualText.matches("ideas")) {
			System.out.println(actualText);
		Assert.assertTrue(actualText.matches("ideas"));
		}
		else {
			System.out.println(actualText);
		Assert.assertTrue(actualText.matches(".*general.*"));
